...
void test_Jsteg_LSBR(const Image &media, const Message &msg,
                     std::string media_ext, std::string msg_ext)
{
    std::cout << "Jsteg LSBR testing:" << std::endl;

    JstegImage jsteg_media = JstegImage(media);
    JstegImage hidden = jsteg_media.Jsteg_LSBR(msg);
    hidden.save_as_jsteg("results/jsteg_lsbr_results/hidden.jsteg");
    Message recovered = hidden.Jsteg_recover(msg.payload);
    recovered.save("results/jsteg_lsbr_results/recovered" + msg_ext);
    ...
    hidden.save("results/jsteg_lsbr_results/lsbs" + media_ext);
}
...
int main(int argc, char *argv[])
{
    ...
    test_Jsteg_LSBR(media, msg, media_ext, msg_ext);
